...
class NN(object):
    ...
    def back_prop_layer(model,l):
        ...
        model.cache[f"dW{l}"] = model.cache[f"dX{l}"].dot(model.cache[f"A{l-1}"].T) / float(batch_size) # Shape: (layer_size, prev_layer_size)
        
        if(model.lmbd!=0):
            d_reg_W = model.deriv_reglr[model.reglr_type](model.W_layers[l]) / float(batch_size) # Shape: (layer_size, prev_layer_size)
            model.cache[f"dW{l}"] = model.cache[f"dW{l}"] + (model.lmbd * d_reg_W * model.cache[f"mask{l}"])
            
            if(model.node_type=='batch_norm'):
                d_sig_W = model.deriv_reglr[model.reglr_type](model.sig_layers[l]) / float(batch_size) # Shape: (layer_size, prev_layer_size)
                model.cache[f"dSig{l}"] = model.cache[f"dSig{l}"] + (model.lmbd * d_sig_W * model.cache[f"mask{l}"])

        if(model.node_type=='standard'):
            model.cache[f"dB{l}"] = np.sum(model.cache[f"dX{l}"],axis=1,keepdims=True) / float(batch_size) # Shape: (layer_size,1)
    ...
    def back_prop_nodes(model,l):
        ...
        if(model.node_type=='batch_norm'):
            ...
            # Shapes: (layer_size, 1)
            model.cache[f"dMu{l}"] = np.sum(model.cache[f"dZ_hat{l}"],axis=1,keepdims=True) / float(batch_size)
            model.cache[f"dSig{l}"] = np.sum(model.cache[f"dZ_hat{l}"] * model.cache[f"Z{l}"],axis=1,keepdims=True) / float(batch_size)

            # Shape: (layer_size, batch_size)
            model.cache[f"dX{l}"] = ((1/float(batch_size)) * model.sig_layers[l] / np.where(model.cache[f"batch_std{l}"]!=0,model.cache[f"batch_std{l}"],np.inf)) * (-model.cache[f"dSig{l}"]*model.cache[f"Z{l}"] + (batch_size)*model.cache[f"dZ_hat{l}"] - model.cache[f"dMu{l}"])
